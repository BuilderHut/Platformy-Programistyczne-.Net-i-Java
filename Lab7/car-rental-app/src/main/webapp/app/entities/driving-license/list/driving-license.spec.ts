...
import { sampleWithRequiredData } from '../driving-license.test-samples';
...
describe('DrivingLicense Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: DrivingLicense;
  let fixture: ComponentFixture<DrivingLicense>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(DrivingLicense);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 11488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.drivingLicenses()[0]).toEqual(expect.objectContaining({ id: 11488 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 11488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.drivingLicenses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 11488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to drivingLicenseService', () => {
      ...
      vitest.spyOn(service, 'getDrivingLicenseIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDrivingLicenseIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.drivingLicensesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
