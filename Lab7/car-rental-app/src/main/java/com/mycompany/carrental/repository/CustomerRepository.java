...
public interface CustomerRepository extends ReactiveCrudRepository<Customer, Long>, CustomerRepositoryInternal {
    ...
    @Query("SELECT * FROM customer entity WHERE entity.driving_license_id = :id")
    Flux<Customer> findByDrivingLicense(Long id);

    @Query("SELECT * FROM customer entity WHERE entity.driving_license_id IS NULL")
    Flux<Customer> findAllWhereDrivingLicenseIsNull();
