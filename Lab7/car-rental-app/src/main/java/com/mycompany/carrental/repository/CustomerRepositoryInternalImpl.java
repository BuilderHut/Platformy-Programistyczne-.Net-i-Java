...
class CustomerRepositoryInternalImpl extends SimpleR2dbcRepository<Customer, Long> implements CustomerRepositoryInternal {
    ...
    private static final Table drivingLicenseTable = Table.aliased("driving_license", "drivingLicense");
    ...
    RowsFetchSpec<Customer> createQuery(Pageable pageable, Condition whereClause) {
        ...
        columns.addAll(DrivingLicenseSqlHelper.getColumns(drivingLicenseTable, "drivingLicense"));
        SelectFromAndJoinCondition selectFrom = Select.builder()
            .select(columns)
            .from(entityTable)
            .leftOuterJoin(drivingLicenseTable)
            .on(Column.create("driving_license_id", entityTable))
            .equals(Column.create("id", drivingLicenseTable));
    ...
    private Customer process(Row row, RowMetadata metadata) {
        ...
        entity.setDrivingLicense(drivinglicenseMapper.apply(row, "drivingLicense"));
        return entity;
    }
