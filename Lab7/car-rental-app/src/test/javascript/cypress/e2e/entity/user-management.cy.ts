...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Beulah58', email: 'Omar.Bergnaum@gmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Brody51');
      cy.get(`[data-cy="login"]`).should('have.value', 'Brody51');

      cy.get(`[data-cy="firstName"]`).type('Wendell');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Wendell');

      cy.get(`[data-cy="lastName"]`).type('Bayer');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Bayer');

      cy.get(`[data-cy="email"]`).type('Rachael_Witting@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Rachael_Witting@hotmail.com');
